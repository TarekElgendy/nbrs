...
class MajorCategoryRequest extends FormRequest
{
    ...
    public function createRules()
    {
        ...
        foreach (config('translatable.locales') as $locale) {
            $this->rules += [$locale . '.title' => ['required', Rule::unique('major_category_translations', 'title')]];
        } //end of for each
    ...
    {
        $item = $this->route('majorCategory');
        foreach (config('translatable.locales') as $locale) {
            $this->rules += [$locale . '.title' => ['required', Rule::unique('major_category_translations', 'title')->ignore($item->id, 'major_category_id')]];
        } // end of  for each
