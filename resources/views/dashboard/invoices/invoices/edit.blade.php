...
<div class="content-wrapper">
    ...
    <div class="no-print">
           Update data selection to change stuts
    </div>
    <!-- Main content -->
        ...
        <div class="row">
            <div class="col-xs-12">
                <h2 class="page-header">
                    <i class="fa fa-globe"></i> <b> @lang('site.invoice_number') </b>: {{ $invoice->invoice_number }}
        ...
        <div class="row invoice-info">
            <div class="col-sm-3 invoice-col">
                <address>
                    ...
                    <b> @lang('site.status') </b>:{{ __('site.'.$invoice->status)}}
                </address>
            </div>
            <!-- /.col -->
            <div class="col-sm-3 invoice-col">
...
{{ $invoice->customer_country=="usd"? __('site.currency_dollar'): __('site.currency_egy')}}
  <del>
                    ...
                    <b> @lang('site.discount') </b>:  <code>{{$invoice->old_price()==0?'':$invoice->old_price()-$invoice->total()}}</code> <br>
        ...
        <div class="row">
            ...
            <div class="col-xs-6">
                <p class="lead"> @lang('site.due_date') : {{ $invoice->due_date }} </p>
...
{{ $invoice->customer_country=="usd"? __('site.currency_dollar'): __('site.currency_egy')}}
                        ...
                        {{ $invoice->old_price()==0?'':$invoice->old_price() }}


                    </del>   <code>{{$invoice->coupon_code}}</code><br>
                    <b> @lang('site.fees') </b>: {{ $invoice->fees }}
{{ $invoice->customer_country=="usd"? __('site.currency_dollar'): __('site.currency_egy')}}
    ...
    <section class="content">
        <div class="box box-default no-print">
            ...
            <div class="box-body">
                @include('partials._errors')
                <form action="{{ route('dashboard.invoices.invoices.update', $invoice->id) }}" method="POST"
