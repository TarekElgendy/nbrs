...
@section('content')
    <div class="content-wrapper">
        <section class="content-header">
            <h1>@lang('dash.cities')</h1>
            <ol class="breadcrumb">
                <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> @lang('site.dashboard')</a>
                </li>
                <li class="active">@lang('dash.cities')</li>
            </ol>
        </section>
        <section class="content">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title" style="margin-bottom: 15px">@lang('dash.cities')
                        <small>
                    ...
                    <form action="{{ route('dashboard.cities.index') }}" method="get">
                        <div class="row">
                            <div class="{{ $city_search == 'close' ? 'hidden' : '' }}">
                                <div class="col-md-4">
                            ...
                            <div class="col-md-4 {{ $city_add == 'close' ? 'hidden' : '' }}">
                                        ...
                                        <td>
                                            @if (auth()->user()->hasPermission('cities-update'))
                                                <a href="{{ route('dashboard.cities.edit', $city->id) }}"
                                                    class="btn btn-info btn-sm {{ $city_edit == 'close' ? 'hidden' : '' }}"><i
                                            ...
                                            @if (auth()->user()->hasPermission('cities-delete'))
                                                <form action="{{ route('dashboard.cities.destroy', $city->id) }}"
                                                    ...
                                                    {{ method_field('delete') }}
                                                    <button type="submit"
                                                        class="btn btn-danger delete btn-sm {{ $city_delete == 'close' ? 'hidden' : '' }}"><i
